...
| 2026-09-26 | 1. First task, tenant and application | Phil: take the first task from ManyFails; candidate triage confirmed. | Tenant `manyfails`, application `research-sweep`, task label `candidate_triage`, English, risk tier low: the daily sweep classifies a headline, description and page excerpt into a strict JSON verdict (failure or not, kind, product, company, date, size, confidence, reason); nothing reaches the public without the ManyFails review queue. The ManyFails triage prompt, schema and acceptance rule are the task definition; dismiss and promote actions in ManyFails admin are the corrections. Extraction is the intended second task. |
| 2026-09-26 | 3. Foundation provider and price list | Phil: OpenRouter; the reviewer chooses the model that works best for the task. | `anthropic/claude-haiku-4.5` via OpenRouter, chosen on the labelled 30-headline fixture (see [the benchmark](evidence/triage-foundation-benchmark-2026-09-26.md)). Price list at decision time: 1.00 USD per million input tokens, 5.00 USD per million output tokens (1,000 and 5,000 micros per 1,000 tokens). Processing region: OpenRouter routes to Anthropic's US endpoints; record `us` on the route candidate. The OpenRouter inference key is the one ManyFails already holds; the platform reads it from its own environment variable, never from a config file. `openai/gpt-6-luna` is the cheap comparison arm. |
| 2026-09-26 | 4. Base model and licence for specialists | Phil: take the default. | `Qwen/Qwen3-1.7B` at Hugging Face commit `70d244cc86ccca08cf5af4e1e306ecf908b1ad5e`, licence Apache-2.0 (permits derivative weights, distillation and pruning). Placed by the reviewer in the host inventory as `base-models/qwen3-1.7b/70d244cc86ccca08cf5af4e1e306ecf908b1ad5e/` with `manifest.json` (model_id `qwen3-1.7b`, tokenizer_id `qwen3-tokenizer-v1`, chat_template_version `qwen3-chat-v1`, licence `Apache-2.0`, SHA-256 per file) and `LICENSE.txt`; not checked into the repository (4.08 GB). CPU-only training on this model needs `Settings.training_memory_limit_bytes` and `training_time_limit_seconds` raised from their synthetic defaults; the smaller `Qwen/Qwen3-0.6B` (same licence) is the fallback if CPU time is prohibitive. |
